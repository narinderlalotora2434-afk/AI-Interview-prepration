...
const CSE_FULLSTACK: Subject[] = [
  {
    id: "frontend-development",
    title: "Frontend Development",
    icon: Layout,
    topics: [
      { id: "fs-1", title: "HTML5 & Semantic Tags", estimatedTime: "1h", difficulty: "Beginner" },
      { id: "fs-2", title: "CSS3 Flexbox & Grid", estimatedTime: "2h", difficulty: "Beginner" },
      { id: "fs-3", title: "JavaScript ES6+ Essentials", estimatedTime: "3h", difficulty: "Intermediate" },
      { id: "fs-4", title: "React Fundamentals (Hooks, Props)", estimatedTime: "4h", difficulty: "Intermediate" },
      { id: "fs-5", title: "Next.js 14/15 App Router", estimatedTime: "4h", difficulty: "Advanced" },
      { id: "fs-6", title: "State Management (Zustand/Redux)", estimatedTime: "3h", difficulty: "Advanced" },
      { id: "fs-7", title: "Tailwind CSS & Styling Systems", estimatedTime: "2h", difficulty: "Intermediate" },
    ]
  },
  {
    ...
    icon: Server,
    topics: [
      { id: "fs-8", title: "Node.js & Express Basics", estimatedTime: "2h", difficulty: "Beginner" },
      { id: "fs-9", title: "RESTful API Design", estimatedTime: "2.5h", difficulty: "Intermediate" },
      { id: "fs-10", title: "Authentication (JWT, OAuth)", estimatedTime: "3h", difficulty: "Advanced" },
      { id: "fs-11", title: "Middleware & Error Handling", estimatedTime: "2h", difficulty: "Intermediate" },
      { id: "fs-12", title: "Server-side Validation", estimatedTime: "1.5h", difficulty: "Intermediate" },
      { id: "fs-13", title: "WebSockets & Real-time Apps", estimatedTime: "3h", difficulty: "Advanced" },
    ]
  },
  {
    id: "database-devops",
    ...
    icon: Cloud,
    topics: [
      { id: "fs-14", title: "SQL (PostgreSQL) vs NoSQL (MongoDB)", estimatedTime: "2h", difficulty: "Intermediate" },
      { id: "fs-15", title: "Prisma / Mongoose ORM", estimatedTime: "2.5h", difficulty: "Intermediate" },
      { id: "fs-16", title: "Git & GitHub Workflow", estimatedTime: "1.5h", difficulty: "Beginner" },
      { id: "fs-17", title: "Docker & Containerization", estimatedTime: "3h", difficulty: "Advanced" },
      { id: "fs-18", title: "CI/CD Pipelines (GitHub Actions)", estimatedTime: "3h", difficulty: "Advanced" },
      { id: "fs-19", title: "Cloud Deployment (AWS/Vercel/Render)", estimatedTime: "2.5h", difficulty: "Advanced" },
    ]
  }
];

export default function CSEFullStackPage() {
  const router = useRouter();
  const [completedTopics, setCompletedTopics] = useState<string[]>([]);
  const [expandedSubject, setExpandedSubject] = useState<string | null>("frontend-development");
  const [mounted, setMounted] = useState(false);
  const [sidebarOpen, setSidebarOpen] = useState(false);

  useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem("cse-fullstack-progress");
    if (saved) {
  ...
  useEffect(() => {
    if (mounted) {
      localStorage.setItem("cse-fullstack-progress", JSON.stringify(completedTopics));
    }
  ...
  const totalTopics = CSE_FULLSTACK.reduce((acc, sub) => acc + sub.topics.length, 0);
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col md:flex-row font-sans relative">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-y-auto relative">
        ...
        <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-white/5 pt-8 pb-4 px-6 md:px-12">
          <Link href="/roadmaps/cse" className="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 font-medium mb-6 transition-colors group">
            <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Back to Software Hub
          </Link>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            <div>
              <h1 className="text-3xl md:text-5xl font-black text-white mb-4 tracking-tight">Full Stack Development</h1>
              <p className="text-lg text-slate-400 max-w-2xl">Master modern web architecture, scalable backends, and cloud deployment for high-growth product roles.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="glass-card p-4 border border-white/10 bg-white/5 flex flex-col items-center justify-center text-center">
              <div className="text-2xl font-black text-emerald-400">{CSE_FULLSTACK.length}</div>
            ...
            <div className="glass-card p-4 border border-white/10 bg-white/5 flex flex-col items-center justify-center text-center relative overflow-hidden">
              <div className="relative z-10">
                <div className="text-2xl font-black text-emerald-500">{progressPercent}%</div>
        ...
        <main className="w-full px-6 md:px-12 py-12">
          <div className="space-y-6">
            {CSE_FULLSTACK.map((subject, sIdx) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`glass-card border overflow-hidden transition-all duration-500 ${isExpanded ? 'border-emerald-500/50 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'border-white/5 hover:border-white/20'}`}
                >
                  {/* Subject Header */}
                  <div 
                    className={`p-6 cursor-pointer flex items-center justify-between transition-colors ${isExpanded ? 'bg-emerald-500/5' : 'bg-white/[0.02] hover:bg-white/[0.04]'}`}
                    onClick={() => setExpandedSubject(isExpanded ? null : subject.id)}
                  >
                    <div className="flex items-center gap-6 flex-1">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border transition-all duration-500 ${allSubjectDone ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400' : 'bg-white/5 border-white/10 text-emerald-400'}`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          {allSubjectDone && (
                            <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase rounded border border-emerald-500/20">Certified Ready</span>
                          )}
                        </div>
                        ...
                        <div className="px-6 pb-6 pt-2 border-t border-white/5 space-y-3 bg-slate-950/30">
                          <div className="flex justify-between items-center mb-4 pt-2 px-2">
                            ...
                            <button 
                              ...
                              className="text-[10px] text-emerald-400 hover:text-emerald-300 font-black uppercase tracking-widest transition-colors flex items-center gap-1.5"
                            >
                            ...
                            {subject.topics.map((topic, tIdx) => {
                              ...
                              return (
                                <motion.div
                                  ...
                                  className={`p-4 rounded-xl border cursor-pointer transition-all duration-300 flex items-center justify-between group ${isDone ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-white/[0.02] border-white/5 hover:bg-white/[0.04] hover:border-emerald-500/30'}`}
                                >
                                  <div className="flex items-center gap-4">
                                    <div className={`w-6 h-6 rounded-lg flex items-center justify-center border transition-all ${isDone ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white/5 border-white/20 text-transparent group-hover:border-emerald-500/50'}`}>
          ...
          <motion.div 
            ...
            className="mt-16 p-10 glass-card text-center border border-white/10 bg-gradient-to-br from-emerald-600/10 to-transparent relative overflow-hidden"
          >
            <div className="relative z-10">
              <Trophy className="w-12 h-12 text-amber-400 mx-auto mb-6" />
              <h3 className="text-2xl font-bold text-white mb-4">Engineering Hero Path</h3>
              <p className="text-slate-400 mb-8 max-w-md mx-auto">Mastering the Full Stack is your gateway to building world-class products and joining high-growth engineering teams.</p>
              <Link href="/roadmaps/cse" className="btn-primary inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 border-emerald-500">
