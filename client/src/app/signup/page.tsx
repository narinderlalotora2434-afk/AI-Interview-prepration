...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Initialization failed. Please verify credentials.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-text-primary selection:bg-primary/10 flex items-center justify-center p-8 relative overflow-hidden">
      {/* Decorative Elements */}
      <div className="absolute top-0 right-0 w-full h-1 bg-gradient-to-r from-accent via-secondary to-primary" />
      <div className="absolute -top-[20%] -right-[10%] w-[600px] h-[600px] bg-primary/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute -bottom-[20%] -left-[10%] w-[600px] h-[600px] bg-secondary/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      >
        <div className="text-center space-y-6">
           ...
           <div className="space-y-3">
             <h1 className="text-5xl font-black tracking-tighter text-text-primary">Create Account</h1>
             <p className="text-text-secondary text-lg font-medium">Join 50,000+ engineers mastering technical interviews.</p>
           </div>
        </div>
        ...
        <div className="saas-card p-12 space-y-10 relative overflow-hidden bg-white shadow-2xl shadow-slate-200/50">
           ...
           <form onSubmit={handleSignup} className="space-y-8 relative z-10">
             <div className="space-y-4">
               <label className="text-[10px] font-black text-text-secondary uppercase tracking-[0.3em] ml-1">Full Name</label>
               <div className="relative group/input">
                 <User className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within/input:text-primary transition-colors" />
                 <input
                   type="text"
                   required
                   value={name}
             ...
             <div className="space-y-4">
               <label className="text-[10px] font-black text-text-secondary uppercase tracking-[0.3em] ml-1">Work Email</label>
               <div className="relative group/input">
             ...
             <div className="space-y-4">
               <label className="text-[10px] font-black text-text-secondary uppercase tracking-[0.3em] ml-1">Security Cipher</label>
               <div className="relative group/input">
             ...
             >
               {loading ? (
                 ...
                 <>
                   <span className="font-black tracking-[0.2em] uppercase text-xs">Initialize Account</span>
        ...
        <p className="text-center text-sm text-text-secondary font-bold">
          Already Registered?{" "}
          <Link href="/login" className="text-primary hover:text-primary/80 font-black transition-all border-b-2 border-primary/20 hover:border-primary">
            Access Neural Link
          </Link>
        </p>
      </motion.div>
